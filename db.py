...
def create_table(table_name):
    c.execute(f"""CREATE TABLE IF NOT EXISTS {table_name}
                  (id TEXT, filename TEXT, label TEXT, apps TEXT, summary TEXT, long_summary TEXT, transcript TEXT, entities TEXT, date TEXT, temp BOOLEAN, hidden BOOLEAN)""")
...
def insert_data(table_name, data):
    ...
    c.execute(f"INSERT INTO {table_name} (id, filename, label, apps, summary, long_summary, transcript, entities, date, temp, hidden) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", tuple(data.values()))
    conn.commit()
